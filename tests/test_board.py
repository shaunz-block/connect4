...
class TestDropDisc:
    ...
    def test_drop_two_discs_same_column(self):
        b = Board()
        b.drop_disc(0, 1)
        row = b.drop_disc(0, 2)
        assert row == b.rows - 2
    ...
    def test_fill_column_then_drop_returns_none(self):
        ...
        b.drop_disc(0, 2)
        b.drop_disc(0, 1)
        result = b.drop_disc(0, 2)
        assert result is None

    def test_player_id_correctly_placed(self):
        b = Board()
        b.drop_disc(3, 2)
...
class TestUndoMove:
    ...
    def test_drop_two_undo_one_removes_top(self):
        ...
        b.drop_disc(0, 2)
        b.undo_move(0)
        # Top disc (row rows-2) should be gone, bottom (row rows-1) still there
        assert b.grid[b.rows - 2][0] == 0
        assert b.grid[b.rows - 1][0] == 1

    def test_drop_undo_drop_lands_same_row(self):
        b = Board()
        row1 = b.drop_disc(0, 1)
        b.undo_move(0)
        row2 = b.drop_disc(0, 2)
        assert row1 == row2
